...
static void reset_rx(struct control_data *data)
{
        data->crc = 0xFFFF;
        data->offset = 0;
        data->state = SLIP_STATE_NORMAL;
}

static void put_byte_to_buffer(struct control_data *data, unsigned char byte)
{
	...
	data->buf[data->offset++] = byte;
	data->crc = calc_crc_ccitt(byte, data->crc);
	data->state = SLIP_STATE_NORMAL;
}

static void slip_read(struct control_data *data, unsigned char *buf,
			   ssize_t len)
{
	for (ssize_t i = 0; i < len; i++) {
		...
		case SLIP_STATE_NORMAL:
			...
			case SLIP_SPECIAL_BYTE_ESC:
				data->state = SLIP_STATE_ESCAPED;
				break;
			default:
				put_byte_to_buffer(data, byte);
				break;
			}
			break;
		...
		case SLIP_STATE_ESCAPED:
			...
			case SLIP_ESCAPED_BYTE_END:
				put_byte_to_buffer(data, SLIP_SPECIAL_BYTE_END);
				break;
			case SLIP_ESCAPED_BYTE_ESC:
				put_byte_to_buffer(data, SLIP_SPECIAL_BYTE_ESC);
				break;
			default:
				reset_rx(data);
				break;
...
static void tty_callback(int fd, uint32_t events, void *user_data)
{
	...
	if (use_slip) {
		len = read(data->fd, buf, sizeof(buf));
		if (len < 0)
			return;
		slip_read(data, buf, len);
	} else {
